...
var latest = require( './fixtures/latest.json' );
...
test( 'if unable to query an endpoint, an error is returned to a provided callback', function test( t ) {
	...
	request( 'dstructs-matrix', opts, clbk );
...
test( 'if an endpoint returns a status code other than 200, an error containing the status code is returned to a provided callback', function test( t ) {
	...
	function clbk( error ) {
		t.equal( error.status, 400, 'equal status codes' );
		t.equal( error.message, 'bad request', 'equal messages' );
		t.end();
	}
});

test( 'if an endpoint returns a status code other than 200 and a message body equal to an empty object, an error containing the status code and a `Not Found` message is returned to a provided callback', function test( t ) {
	var request;
	var mock;
	var opts;
	...
	function clbk( error ) {
		...
		t.equal( error.message, 'Not Found', 'returns an error with a `Not Found` message' );
		t.end();
	}
});
...
test( 'the option to restrict package info to only the latest registry entry is supported', function test( t ) {
	var request;
	var mock;
	var opts;
